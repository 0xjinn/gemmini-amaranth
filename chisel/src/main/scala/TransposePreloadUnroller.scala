...
class TransposePreloadUnroller[T <: Data, U <: Data, V <: Data](config: GemminiArrayConfig[T, U, V], xLen: Int) extends Module {
  ...
  val io = IO(new Bundle {
    val in = Flipped(Decoupled(new GemminiCmd(xLen, reservation_station_entries)))
    val out = Decoupled(new GemminiCmd(xLen, reservation_station_entries))
...
object TransposePreloadUnroller {
  def apply[T <: Data, U <: Data, V <: Data](in: ReadyValidIO[GemminiCmd], config: GemminiArrayConfig[T, U, V], counter: CounterEventIO, xLen: Int): DecoupledIO[GemminiCmd] = {
    val mod = Module(new TransposePreloadUnroller(config, xLen))
    mod.io.in <> in
