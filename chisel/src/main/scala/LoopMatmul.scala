...
class LoopMatmulLdA(block_size: Int, coreMaxAddrBits: Int, iterator_bitwidth: Int, max_addr: Int, input_w: Int,
                    max_block_len: Int, concurrent_loops: Int, mvin_rs2_t: MvinRs2, xLen: Int)
                    extends Module {
...
class LoopMatmulLdD(block_size: Int, coreMaxAddrBits: Int, iterator_bitwidth: Int, max_acc_addr: Int, input_w: Int,
                    acc_w: Int, max_block_len: Int, max_block_len_acc: Int, concurrent_loops: Int, mvin_rs2_t: MvinRs2, xLen: Int)
                    extends Module {
  val io = IO(new Bundle {
...
class LoopMatmulExecute(block_size: Int, coreMaxAddrBits: Int, iterator_bitwidth: Int, max_addr: Int, max_acc_addr: Int, concurrent_loops: Int,
                        ...
                        compute_rs1_t: ComputeRs, compute_rs2_t: ComputeRs, xLen: Int)
                        extends Module {
  val io = IO(new Bundle {
  ...
  val pre_cmd = Wire(new RoCCCommand(xLen))
  pre_cmd := DontCare
  ...
  val comp_cmd = Wire(new RoCCCommand(xLen))
  comp_cmd := DontCare
...
class LoopMatmulStC(block_size: Int, coreMaxAddrBits: Int, iterator_bitwidth: Int, max_acc_addr: Int, input_w: Int, acc_w: Int, max_block_len: Int, concurrent_loops: Int, mvout_rs2_t: MvoutRs2, xLen: Int)
                    extends Module {
  val io = IO(new Bundle {
  ...
  val ln_config_norm = Wire(new RoCCCommand(xLen))
  ...
  val ln_mvout_cmd = Wire(new RoCCCommand(xLen))
...
class LoopMatmul(block_size: Int, coreMaxAddrBits: Int, reservation_station_size: Int, max_lds: Int, max_exs: Int, max_sts: Int,
                 ...
                 compute_rs1_t: ComputeRs, compute_rs2_t: ComputeRs, mvout_rs2_t: MvoutRs2, xLen: Int)
                 extends Module {
  ...
  val ldA = Module(new LoopMatmulLdA(block_size, coreMaxAddrBits, iterator_bitwidth, max_all_addr, input_w, max_block_len, concurrent_loops, mvin_rs2_t, xLen))
  val ldB = Module(new LoopMatmulLdB(block_size, coreMaxAddrBits, iterator_bitwidth, max_all_addr, input_w, max_block_len, concurrent_loops, mvin_rs2_t, xLen))
  val ldD = Module(new LoopMatmulLdD(block_size, coreMaxAddrBits, iterator_bitwidth, max_acc_addr, input_w, acc_w, max_block_len, max_block_len_acc, concurrent_loops, mvin_rs2_t, xLen))
  val ex = Module(new LoopMatmulExecute(block_size, coreMaxAddrBits, iterator_bitwidth, max_addr, max_acc_addr, concurrent_loops, preload_rs1_t, preload_rs2_t, compute_rs1_t, compute_rs2_t, xLen))
  val stC = Module(new LoopMatmulStC(block_size, coreMaxAddrBits, iterator_bitwidth, max_acc_addr, input_w, acc_w, max_block_len, concurrent_loops, mvout_rs2_t, xLen))
  ...
  // Create ld arbiters
  val ldab_arb = Module(new WeightedArbiter(new RoCCCommand(xLen), maxWeightA=255, staticWeightAEnabled=true)) // TODO magic numbers
  ...
  val arb = Module(new Arbiter(new RoCCCommand(xLen), 4))
...
object LoopMatmul {
  def apply(in: DecoupledIO[GemminiCmd], ld_completed: UInt, st_completed: UInt, ex_completed: UInt,
    ...
    val mod = Module(new LoopMatmul(block_size, coreMaxAddrBits, rob_size, max_lds, max_exs, max_sts,
      ...
      mvin_rs2_t, preload_rs1_t, preload_rs2_t, compute_rs1_t, compute_rs2_t, mvout_rs2_t, xLen))
    mod.io.in <> in
