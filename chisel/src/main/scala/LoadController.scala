...
class LoadController[T <: Data, U <: Data, V <: Data](config: GemminiArrayConfig[T, U, V], coreMaxAddrBits: Int, local_addr_t: LocalAddr) extends Module {
  import config._

  val io = IO(new Bundle {
    val cmd = Flipped(Decoupled(new GemminiCmd(xLen, reservation_station_entries)))

    val dma = new ScratchpadReadMemIO(local_addr_t, mvin_scale_t_bits, coreMaxAddrBits)
