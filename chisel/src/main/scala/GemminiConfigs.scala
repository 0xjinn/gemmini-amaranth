...
case class GemminiArrayConfig[T <: Data : Arithmetic, U <: Data, V <: Data](
  ...
  lazy val mvin_scale_t_bits = mvin_scale_t.getWidth max mvin_scale_acc_t.getWidth
  ...
  lazy val acc_scale_t_bits = acc_scale_t.getWidth
...
"""
  ...
  def generateJson(): String = {
    ...
    val write_data_width = if (acc_read_full_width) acc_w else spad_w

    val maxBytes = dma_maxbytes
    val nXacts = max_in_flight_mem_reqs
    val beatBits = dma_buswidth
    val vaddrBits = coreMaxAddrBits

    // Packet field widths (replicating Bundle constructors)
    val readReq_lgSize_w = log2Ceil(log2Ceil(maxBytes + 1))
    val readReq_bytesRead_w = log2Up(maxBytes + 1)
    val readReq_shift_w = log2Up(maxBytes)
    val readReq_source_w = log2Up(nXacts)

    val readResp_source_w = log2Up(nXacts)
    val readResp_lgSize_w = log2Up(log2Up(maxBytes + 1) + 1)

    val writeResp_lgSize_w = log2Up(log2Up(maxBytes + 1) + 1)
    val writeResp_source_w = log2Up(nXacts)

    val typeFamily = inputType match {
      case _: SInt => "sint"
      case _: Float => "float"
      case _: DummySInt => "dummy"
      case _ => "unknown"
    }
    ...
    s"""{
       ...
       |  "inputWidthBits": ${inputType.getWidth},
       |  "accWidthBits": ${accType.getWidth},
       |  "dataflow": "${dataflow}",
       |  "dmaBuswidth": $dma_buswidth,
       |  "dmaMaxbytes": $dma_maxbytes,
       |  "maxInFlightMemReqs": $max_in_flight_mem_reqs,
       |  "vaddrWidth": $vaddrBits,
       |  "accReadFullWidth": $acc_read_full_width,
       |  "writeDataWidth": $write_data_width,
       |  "spadWidth": $spad_w,
       |  "accWidth": $acc_w,
       |  "spBanks": $sp_banks,
       |  "spBankEntries": $sp_bank_entries,
       |  "accBanks": $acc_banks,
       |  "accBankEntries": $acc_bank_entries,
       |  "readReqPacket": {
       |    "lgSizeWidth": $readReq_lgSize_w,
       |    "bytesReadWidth": $readReq_bytesRead_w,
       |    "shiftWidth": $readReq_shift_w,
       |    "sourceWidth": $readReq_source_w,
       |    "vaddrWidth": $vaddrBits
       |  },
       |  "readRespPacket": {
       |    "sourceWidth": $readResp_source_w,
       |    "dataWidth": $beatBits,
       |    "lgSizeWidth": $readResp_lgSize_w
       |  },
       |  "writeReqPacket": {
       |    "dataWidth": $write_data_width,
       |    "vaddrWidth": $vaddrBits
       |  },
       |  "writeRespPacket": {
       |    "lgSizeWidth": $writeResp_lgSize_w,
       |    "sourceWidth": $writeResp_source_w
       |  }
       |}""".stripMargin
  }
