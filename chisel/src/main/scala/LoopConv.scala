...
class LoopConvLdBias(block_size: Int, coreMaxAddrBits: Int, large_iterator_bitwidth: Int, small_iterator_bitwidth: Int, tiny_iterator_bitwidth: Int, max_acc_addr: Int, acc_w: Int,
                     ...
                     config_mvin_rs1_t: ConfigMvinRs1, mvin_rs2_t: MvinRs2, xLen: Int) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val cmd = Decoupled(Output(new RoCCCommand(xLen)))
  ...
  // Commands
  val config_cmd = Wire(new RoCCCommand(xLen))
  config_cmd := DontCare
  ...
  val mvin_cmd = Wire(new RoCCCommand(xLen))
  mvin_cmd := DontCare
...
class LoopConvLdInput(block_size: Int, coreMaxAddrBits: Int, large_iterator_bitwidth: Int, small_iterator_bitwidth: Int,
                      ...
                      concurrent_loops: Int, latency: Int, config_mvin_rs1_t: ConfigMvinRs1, mvin_rs2_t: MvinRs2, xLen: Int)
                     extends Module {
  val MVIN_SCALE_IDENTITY = 0x3f800000.U // TODO get this from configs somehow
...
class LoopConvLdWeight(block_size: Int, coreMaxAddrBits: Int, large_iterator_bitwidth: Int,
                       ...
                       mvin_rs2_t: MvinRs2, xLen: Int) extends Module {
  val MVIN_SCALE_IDENTITY = 0x3f800000.U // TODO get this from configs somehow
...
class LoopConvExecute(block_size: Int, large_iterator_bitwidth: Int, small_iterator_bitwidth: Int, tiny_iterator_bitwidth: Int, max_addr: Int,
                      ...
                      compute_rs1_t: ComputeRs, compute_rs2_t: ComputeRs, xLen: Int) extends Module {
  val io = IO(new Bundle {
  ...
  val pre_cmd = Wire(new RoCCCommand(xLen)) // preload
  pre_cmd := DontCare
  pre_cmd.inst.funct := PRELOAD_CMD
  pre_cmd.rs1 := 0.U//(K << 48) | (J << 32) | pre_addr
  pre_cmd.rs2 := 0.U//(I << 48) | (J << 32) | c_addr

  val comp_cmd = Wire(new RoCCCommand(xLen)) // compute.preloaded
  comp_cmd := DontCare
...
class LoopConvSt(block_size: Int, coreMaxAddrBits: Int, large_iterator_bitwidth: Int, small_iterator_bitwidth: Int, tiny_iterator_bitwidth: Int, max_acc_addr: Int, input_w: Int,
  concurrent_loops: Int, latency: Int, config_mvout_rs2_t: ConfigMvoutRs2, mvout_rs2_t: MvoutRs2, xLen: Int) extends Module {
  ...
  val command_p = Module(new Pipeline[RoCCCommandWithAddr](new RoCCCommandWithAddr, latency)())
  // Commands
  val mvout_cmd = Wire(new RoCCCommand(xLen))
  mvout_cmd := DontCare
  mvout_cmd.inst.funct := STORE_CMD
  mvout_cmd.rs1 := 0.U // dram_addr
  mvout_cmd.rs2 := 0.U // mvout_cmd_rs2

  val pre_pool_config_cmd = Wire(new RoCCCommand(xLen))
  ...
  val post_pool_config_cmd = Wire(new RoCCCommand(xLen))
  ...
  val pool_cmd = Wire(new RoCCCommand(xLen))
  pool_cmd := DontCare
...
class LoopConv (block_size: Int, coreMaxAddrBits: Int, reservation_station_size: Int, max_lds: Int, max_exs: Int, max_sts: Int,
                ...
                has_dw_convs: Boolean, xLen: Int) extends Module {
  ...
  val io = IO(new Bundle {
    val in = Flipped(Decoupled(new GemminiCmd(xLen, reservation_station_size)))
    val out = Decoupled(new GemminiCmd(xLen, reservation_station_size))
  ...
  val latency = 2
  val ld_bias = Module(new LoopConvLdBias(block_size, coreMaxAddrBits, large_iterator_bitwidth, small_iterator_bitwidth, tiny_iterator_bitwidth, max_acc_addr, acc_w, max_block_len_acc, concurrent_loops, latency, config_mvin_rs1_t, mvin_rs2_t, xLen))
  val ld_input = Module(new LoopConvLdInput(block_size, coreMaxAddrBits, large_iterator_bitwidth, small_iterator_bitwidth, tiny_iterator_bitwidth, max_addr, input_w, max_block_len, concurrent_loops, latency, config_mvin_rs1_t, mvin_rs2_t, xLen))
  val ld_weights = Module(new LoopConvLdWeight(block_size, coreMaxAddrBits, large_iterator_bitwidth, small_iterator_bitwidth, tiny_iterator_bitwidth, max_addr, input_w, max_block_len, concurrent_loops, latency, config_mvin_rs1_t, mvin_rs2_t, xLen))
  val ex = Module(new LoopConvExecute(block_size, large_iterator_bitwidth, small_iterator_bitwidth, tiny_iterator_bitwidth, max_addr, max_acc_addr, concurrent_loops, latency, config_ex_rs1_t, preload_rs1_t, preload_rs2_t, compute_rs1_t, compute_rs2_t, xLen))
  val st = Module(new LoopConvSt(block_size, coreMaxAddrBits, large_iterator_bitwidth, small_iterator_bitwidth, tiny_iterator_bitwidth, max_acc_addr, input_w, concurrent_loops, latency, config_mvout_rs2_t, mvout_rs2_t, xLen))
  ...
  // Create arbiter
  val arb = Module(new Arbiter(new RoCCCommand(xLen), 5))
...
object LoopConv {
  def apply(in: DecoupledIO[GemminiCmd], ld_completed: UInt, st_completed: UInt, ex_completed: UInt,
            ...
            has_first_layer_optimizations: Boolean, has_dw_convs: Boolean, xLen: Int) : (DecoupledIO[GemminiCmd], Bool) = {
    ...
    val mod = Module(new LoopConv(block_size, coreMaxAddrBits, rob_size, max_lds, max_exs, max_sts,
      ...
      compute_rs1_t, compute_rs2_t, has_training_convs, has_max_pool, has_first_layer_optimizations, has_dw_convs, xLen))
