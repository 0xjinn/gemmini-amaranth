...
class CounterController(nPerfCounter: Int, xLen: Int) extends Module {
  val io = IO(new Bundle() {
    val in = Flipped(Decoupled(new RoCCCommand(xLen)))
    val out = Decoupled(new RoCCResponse(xLen))
    val event_io = Flipped(new CounterEventIO)
  })
  ...
  if (nPerfCounter > 0) {
    ...
    val counterfile = Module(new CounterFile(nPerfCounter, xLen))
